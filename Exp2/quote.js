...
var block1 = '<br> <button id = \"test\" class=\"button btn btn-danger\"> Nothingness </button> <a class=\"button\" target=\"_blank\" href=\'https://twitter.com/intent/tweet?text=';

var block2 = '\'> <button class = "button btn btn-primary"> Tweet the quote! </button></a> <button id = "quotebutton" class="button btn btn-primary">Random Quotes</button> <br> <span class = "sourcelink"> Source of the quotes: <br> <a href = "http://buddhaimonia.com/101-zen-sayings-and-proverbs/">http://buddhaimonia.com/101-zen-sayings-and-proverbs/</a></span>';
...
$(document).ready(function() {
  ...
  $('.response').on('click','#test', function(){
    $(".response").html('poof!')
  });

  $('.response').on('click', '#quotebutton', function() {
    qii = Math.floor(Math.random()*(qlen)) ;
    if (qii >= quotes.length) {
      qii = 1;
    }
     quoteblock = [quotes[qii],block1,quotes[qii],block2];
     qblock = "\'" + quoteblock.join('')+ "\'";
     console.log(qblock);

    $(".response").html(qblock);
  });
});
